...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
float RAD_TO_DEG(float x) { return x * 57.2958f; }
...
void Read_LSM() {
  // high[0] = xh, high[1] = yh, high[2] = zh
  // low[0] = xl, low[1] = yl, low[2] = zl
  uint8_t high[3];
  uint8_t low[3];
...
void Offset_LSM() {
  ...
  offset[1] = temp_acc[1] / 20.0f;
  offset[2] = temp_acc[2] / 20.0f;
}
